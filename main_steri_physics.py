...
parser.add_argument(
    "bop_parent_path", nargs="?", help="Path to the bop datasets parent directory"
)
parser.add_argument(
    "cc_textures_path",
    nargs="?",
...
sampled_bop_objs = bproc.loader.load_bop_objs(
    bop_dataset_path=os.path.join("bop_datasets", "steri"),
...
bproc.loader.load_bop_intrinsics(bop_dataset_path=os.path.join("bop_datasets", "steri"))
...
for j, obj in enumerate(sampled_bop_objs + distractor_bop_objs):
    ...
    b_col = np.random.uniform(0.5, 0.9)
...
def sample_pose_func(obj: bproc.types.MeshObject):
    min = np.random.uniform([-0.1, -0.1, 0.0], [-0.1, -0.1, 0.0])
    max = np.random.uniform([0.1, 0.1, 0.4], [0.1, 0.1, 0.4])
...
bproc.object.sample_poses(
    objects_to_sample=sampled_bop_objs + distractor_bop_objs,
...
bop_bvh_tree = bproc.object.create_bvh_tree_multi_objects(
    sampled_bop_objs + distractor_bop_objs
)

# TODO use n_scenes like in other scripts and sample object here
poses = 0
while poses < args.views_per_scene:
    # Sample location
    location = bproc.sampler.shell(
        center=[0, 0, 0],
        radius_min=0.61,

...
target_bop_objs = bproc.loader.load_bop_objs(
    bop_dataset_path=os.path.join(args.bop_parent_path, "lmo"), object_model_unit="mm"
)
...
bproc.loader.load_bop_intrinsics(
    bop_dataset_path=os.path.join(args.bop_parent_path, "lmo")
)
...
for i in range(args.num_scenes):
    ...
    sampled_target_bop_objs = list(
        np.random.choice(target_bop_objs, size=8, replace=False)
    )
    ...
    while cam_poses < args.views_per_scene:
        ...
        poi = bproc.object.compute_poi(
            np.random.choice(sampled_target_bop_objs, size=8, replace=False)
        )
